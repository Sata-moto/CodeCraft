...
bool Car::ChooseAvoider(int Cnum) {
	...
	if ((!car[Cnum].FindAvoid || (car[Cnum].FindAvoid && car[numm].Avoidnum == numID))
...
pair<double, double> Car::Static_Avoidance(int desk_num, int mode) {
	...
	*/
	...
	while (startang < endang) {
		assist_n::check_jump_frame(5);
		maxlen = -1; l = 0; r = 5;
		while (r - l >= 0.125) {
			assist_n::check_jump_frame(6);
			mid = (l + r) / 2;
...
pair<double, double> Car::Dynamic_Avoidance(int mode) {
	...
	{
		output << "numm is " << numm << endl;
		assist_n::check_jump_frame(8);
	...
	while (startang < endang) {
		...
		while (r - l >= 0.01) {
			...
			if (ObCheck(sx, sy, tx, ty, destination[Avoidnum], GetR(goods), 0))maxlen = mid, l = mid;
			else r = mid;
		}
		...
		if (fabs(goodPoint.first + 1) > eps && Dist(goodPoint, realT) > 1.2) {
			setto = make_pair(x + (Dist(realS, goodPoint) + 1.2) * cos(startang),
				y + (Dist(realS, goodPoint) + 1.2) * sin(startang));
			FindAvoid = 2;
			break;
		}
		startang += deltaang;
